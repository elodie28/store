...
//        $jeweler = $em->getRepository('StoreBackendBundle:Jeweler')->find($user); // Je récupère le jeweler connecté
    ...
    public function removeAction($id) {
        ...
        // Je récupère 1 slide avec la méthode find()
        $slider = $em->getRepository('StoreBackendBundle:Slider')->find($id); // NomduBundle:Nomdel'entité

        $em->remove($slider); // supprime le slide
        ...
        return $this->redirectToRoute('store_backend_slider_list'); // redirection vers la liste des slides
    ...
    /**
     * Action d'activation d'un slide dans la page liste des slides
     * @param $id
     * @return \Symfony\Component\HttpFoundation\Response
     */
    public function activateAction(Slider $id, $action){
        ...
        $id->setActive($action); // J'associe l'action activate à l'id de mon slide
        $em->persist($id); // J'enregistre l'id du slide dans Doctrine
        $em->flush(); // J'envoie ma requête  à ma table slider

        // Permet d'écrire un message flash avec pour clef "info" et un message de confirmation
        $this->get('session')->getFlashBag()->add(
            'info',
            'Votre slide a bien été désactivé'
        );
    ...
    /**
     * Action de désactivation d'un slide dans la page liste des slides
     * @param $id
     * @return \Symfony\Component\HttpFoundation\Response
     */
    public function desactivateAction(Slider $id, $action){

        // Je récupère le manager de doctrine : le conteneur d'objets de Doctrine
        $em = $this->getDoctrine()->getManager();

        $id->setActive($action); // J'associe l'action desactivate à l'id de mon slide
